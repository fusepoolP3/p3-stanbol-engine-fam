...
public class Fise2FamEngineTest {
    ...
    private static final String TEST_KEY_SENT_ENHANCEMENTS = "keysentenhancements.ttl";
    private static final String TEST_KEY_SENT_FILE = "keysentcontent.txt";
    ...
    //Keyword and Sentiment Enhancements
    private static final int KS_NUM_SENTIMENT = 38;
    private static final int KS_NUM_KEYWORD = 10;
    private static MGraph ksOrigEnhancements;
    private static UriRef ksCiUri;
    private static String ksContent;
    private ContentItem ksContentItem;
    ...
    public void initContentItem() throws IOException{
        ...
        ksContentItem = ciFactory.createContentItem(ksCiUri, new StringSource(ksContent),
                new IndexedMGraph(ksOrigEnhancements));
    }
    ...
    public void testDefaultEngineConfig() throws EngineException, ConfigurationException{
        ...
        assertTransformationResults(contentItem);
    }
    ...
    @Test
    public void testKeywordAndSentimentAnnotations() throws EngineException, ConfigurationException{
        log.info("> test Keyword- and SentimentAnnotation transformation");
        activate(null, true);
        Assert.assertNotEquals(EnhancementEngine.CANNOT_ENHANCE, engine.canEnhance(ksContentItem));
        engine.computeEnhancements(ksContentItem);
        //log the transformed RDF
        logRdf(" - transformed RDF: \n",ksContentItem.getMetadata());
        //assert EntityMentions
        assertKeywordAndSentimentResults(ksContentItem);
    }
    
    private void assertKeywordAndSentimentResults(ContentItem contentItem) {
        TripleCollection graph = contentItem.getMetadata();
        Iterator<Triple> it = graph.filter(null, RDF_TYPE, Constants.FAM_KEYWORD_ANNOTATION);
        int numKeyAnno = 0;
        while(it.hasNext()){
            NonLiteral body = it.next().getSubject();
            assertAnnotation(contentItem, body); //asserts oa:Annotation, incl selectors and source
            PlainLiteral keyword = assertSingleValue(graph, body, Constants.FAM_KEYWORD, PlainLiteral.class);
            Assert.assertFalse(keyword.getLexicalForm().trim().isEmpty());
            Assert.assertEquals(CONTENT_LANGUAGE, keyword.getLanguage());
            
            TypedLiteral metricLteral = assertSingleValue(graph, body, Constants.FAM_METRIC, TypedLiteral.class);
            Assert.assertEquals(XSD.double_, metricLteral.getDataType());
            Double metric = lf.createObject(Double.class, metricLteral);
            assertTrue(metric > 0.0 && metric <= 1.0);
            
            TypedLiteral countLiteral = assertSingleValue(graph, body, Constants.FAM_COUNT, TypedLiteral.class);
            Assert.assertEquals(XSD.int_, countLiteral.getDataType());
            Integer count = lf.createObject(Integer.class, countLiteral);
            assertTrue(count >= 1);
            
            numKeyAnno++;
        }
        Assert.assertEquals(KS_NUM_KEYWORD, numKeyAnno);
        
        int numSentAnno = 0;
        it = graph.filter(null, RDF_TYPE, Constants.FAM_SENTIMENT_ANNOTATION);
        while(it.hasNext()){
            NonLiteral body = it.next().getSubject();
            assertAnnotation(contentItem, body); //asserts oa:Annotation, incl selectors and source
            TypedLiteral sentimentLiteral = assertSingleValue(graph, body, Constants.FAM_SENTIMENT, TypedLiteral.class);
            Assert.assertEquals(XSD.double_, sentimentLiteral.getDataType());
            Double sentiment = lf.createObject(Double.class, sentimentLiteral);
            assertTrue(sentiment >= -1.0 && sentiment <= 1.0);
            numSentAnno++;
        }
        Assert.assertEquals(KS_NUM_SENTIMENT, numSentAnno);
        it = graph.filter(null, RDF_TYPE, Constants.FAM_DOCUMENT_SENTIMENT_ANNOTATION);
        Assert.assertTrue(it.hasNext());
        NonLiteral body = it.next().getSubject();
        assertAnnotation(contentItem, body); //asserts oa:Annotation, incl selectors and source
        TypedLiteral sentimentLiteral = assertSingleValue(graph, body, Constants.FAM_SENTIMENT, TypedLiteral.class);
        ...
        Double sentiment = lf.createObject(Double.class, sentimentLiteral);
        assertTrue(sentiment >= -1.0 && sentiment <= 1.0);
        Assert.assertFalse(it.hasNext()); //only a single DocumentSentimentAnnotation expected
        
    }

    private void assertTransformationResults(ContentItem contentItem) {
        TripleCollection graph = contentItem.getMetadata();
        ...
        assertAnnotation(contentItem, langAnnoBody);
        ...
        while(it.hasNext()){
            NonLiteral body = it.next().getSubject();
            assertAnnotation(contentItem, body);
     ...
     * @param object
     */
    private void assertAnnotation(ContentItem contentItem, Resource annoBodyRes) {
        TripleCollection graph = contentItem.getMetadata();
        ...
        for(UriRef selector : bodySelector){
            if(engine.selectorType == SelectorTypes.NIF || engine.selectorType == SelectorTypes.BOTH){
                assertNifSelector(contentItem, selector);
            }
            if(engine.selectorType == SelectorTypes.OA || engine.selectorType == SelectorTypes.BOTH) {
                assertOASelector(contentItem, selector);
            }
        }
    }

    private void assertSelectorUri(ContentItem contentItem, UriRef selector, int start, int end){
